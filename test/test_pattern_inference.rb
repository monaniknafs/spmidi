...
module SPMidi
  class TestPatternInference < Test::Unit::TestCase
    def test_find_pattern_size
      ...
      assert_equal(pattern_notes1.length, pi1.current.length)

      for i in 0..pi1.current.length-1
        assert_equal(pattern_notes1[i].rel_ts, pi1.current.notes[i].rel_ts)
        assert_equal(pattern_notes1[i].data, pi1.current.notes[i].data)
      end
